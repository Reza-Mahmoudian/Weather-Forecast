...
function getApiInfo(city) {
    var weatherAPI = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=bef2e0ca16b5b0c3200c0b21566a0d6e";
    fetch(weatherAPI)
	.then(response => {
        if(response.ok){
...
function getUVIndex(lon,lat) {
    var allinOneAPI = "https://api.openweathermap.org/data/2.5/onecall?lat="+lat+"&lon="+lon+"&exclude=hourly,minutely&appid=bef2e0ca16b5b0c3200c0b21566a0d6e";
    fetch(allinOneAPI)
	.then(response => {
        if(response.ok){
            response.json().then(data => {
